...
int ParseTransportableItemKuge(AString *token) {
    int r = -1;
    for (int i=0; i<NITEMS; i++) {
	if(ItemDefs[i].flags & ItemType::DISABLED) continue;
	if(ItemDefs[i].flags & ItemType::CANTGIVE) continue;
	...
	//move weapon
	else if(ItemDefs[i].type & IT_WEAPON) {
	    if ((*token == (AString("i") + ItemDefs[i].name)) ||
		(*token == (AString("i") + ItemDefs[i].names)) ||
		(*token == (AString("i") + ItemDefs[i].abr))) {
		r = i;
	...
	//move mount
	else if(ItemDefs[i].type & IT_MOUNT) {
	    if ((*token == (AString("i") + ItemDefs[i].name)) ||
		(*token == (AString("i") + ItemDefs[i].names)) ||
		(*token == (AString("i") + ItemDefs[i].abr))) {
		r = i;
	...
	//move silver
	else if(ItemDefs[i].type & IT_MONEY) {
	    if ((*token == (AString("i") + ItemDefs[i].name)) ||
		(*token == (AString("i") + ItemDefs[i].names)) ||
		(*token == (AString("i") + ItemDefs[i].abr))) {
		r = i;
...
AString ShowSpecial(char *special, int level, int expandLevel, int fromItem)
{
	...
	temp += AString(val);
	if(!expandLevel) {
	    temp += " times the skill level of the unit";
	}
...
AString *ItemDescription(int item, int full)
{
    ...
    *temp += ".";
    ...
    //weapons
    if (item == I_YARI) {
      *temp += " This is the common spear.";
    } else if (item == I_NO_DACHI || item == I_NO_DACHI_MASTERWORK || item == I_NO_DACHI_MAGICAL) {
      *temp += " This is a two handed sword.";
    } else if (item == I_NAGINATA) {
      *temp += " This is a weapon consisting of a polearm with a curved blade at its end.";
    } else if (item == I_DAI_KYU || item == I_DAI_KYU_MASTERWORK || item == I_DAI_KYU_MAGICAL) {
      *temp += " This is a heavy bow the height of a man requiring high strength to draw and fire.";
    } else if (item == I_HAN_KYU) {
      *temp += " This is a common small bow.";
    } else if (item == I_DAISHO || item == I_DAISHO_MASTERWORK || item == I_DAISHO_MAGICAL) {
      *temp += " This is a combination of two weapons wielded simutaneously: a"
    ...
    //monsters
    if (item == I_DAI_ONI) {
      *temp += " Dai-Oni are Supernatural Beings bridging the"
	" gap between the Spirits of the invisible realms and the mortal"
	" creatures of this earth. Dai-oni, or Oni Kings, as they"
	" may be called, rule numerous Oni. They dress in stolen finery,"
	" which only serves to make their grotesque appearance pathetically"
	" funny as well as horrible. They are larger than the average Oni.";
    } else  if (item == I_ONI) {
      *temp += " Oni are Supernatural Beings bridging the gap between the"
	" Spirits of the invisible realms and the mortal creatures of this"
	" earth. Oni range in height from 1 to 2 metres tall. They are hairy"
	" about the face, not so much bearded as hirsute, with ragged, unkempt"
	" hair. Their skins are rough, darkened by sun and wind, though human" 
	" enough in texture. Their hands are clawed.";
    } else if (item == I_OBAKEMONO) {
      *temp += " 0-bakemono are 6 to 8 feet tall, dumb and resembling western"
	" style ogres. They are uncouth and unkept."
	" Most are smooth-skinned, but some are scaled. All tend to wear"
	" ill-smelling, patched clothes or skins.";
    } else if (item == I_SHIKOME) {
      *temp += " Shikome are humanoid and stand 4 to 5 feet tall."
	" Their skin tends to be dusky and is often covered with warts."
	" Though their personal habits are uncouth, they take meticulous"
	" care of their weapons.";
    } else if (item == I_KAPPA) {
      *temp += " The Kappa is a water being. It is usually scaled over"
	" its entire body, though sometimes one is relatively smooth-skinned."
	" The Kappa has a bowl-shaped depression on the top of its rather"
	" flat head.";
    } else if (item == I_MINOR_KAMI || item == I_KAMI || item == I_MAJOR_KAMI) {
      *temp += " Kami are magical spirits that inhabit this world.";
    } else if (item == I_KITSUNE) {
      *temp += " The Kitsune is a fox spirit. In fox form it looks like a"
	" long furred fox. More powerful Kitsune often have more than one"
	" tail, the most powerful of them having as many as nine."
	" They can change form making them very hard to notice.";
    } else if (item == I_BAKEMONO_SHO) {
      *temp += " The Bakemono-sho is a humanoid goblin. Its skin is warty"
	" though of human hue. Bakemono-sho are often unkempt and usually"
	" far from polite.";
    } else if (item == I_YAMATOKAGE) {
      *temp += " The Yamatokage is a gigantic, heavily-scaled, predatory"
	" lizard. It reaches a length of 30 feet, of which one-third is tail.";
    } else if (item == I_KUMO) {
      *temp += " This is a monster spider.";
    } else if (item == I_YAMABUSHI) {
      *temp += " The yamabushi are priests who also have a warrior"
	" orientation. Given their warlike ways and their taste for a hot time"
	", which includes drinking, gambling, wrestling, and even dallying"
	" with courtesans, they have lost their magical abilities.";
    } else if (item == I_RONIN) {
      *temp += " The ronin are masterless samurai who have taken to crime to fund their lifestyle.";
    } else if (item == I_SHUGENJA) {
      *temp += " The Shugenja will rarely be clad in any distinctive manner, preferring the garb of their caste or that suitable to a Scholar.";
    } else if (item == I_PILGRIMS) {
      *temp += " Pilgrims are groups of fanatical worshipers who won't let anyone stand in their way.";
    } else if (item == I_BANDITS) {
      *temp += " Bandits are petty thieves, plying the robber's trade on the highways.";
    } else if (item == I_GAKI) {
      *temp += " Gaki appear as gaunt-faced monsters with enormous, frog-like"
	" mouths and grossly distended bellies. They can also appear in the" 
	" form they bore in life if they died with some overwhelming" 
	" need or desire unfulfilled: hunger, greed, lust or revenge.";
    } else if (item == I_HOU_OU) {
      *temp += " This is a noble creature, similar to a pheonix.";
    }
	...
	*temp += " This race may study ";
	...
	if(found) {
	    ...
	    *temp += AString("all skills to level ") + mt->defaultlevel + AString(".");
	}
    }
	...
	*temp += "silver as treasure.";
    ...
    //BUSHIDO artifact mod
    if(ItemDefs[item].type & IT_MONSTER_DROP) {
      *temp += " This item is part of the remains of a";

      if (item == I_ONI_HORN || item == I_ONI_HIDE || item == I_ONI_BLOOD)
	*temp += " dai-oni";
      if (item == I_KAMI_RIB || item == I_KAMI_HAIR || item == I_KAMI_HEART)
	*temp += " major kami";
      if (item == I_GAKI_ICHOR || item == I_GAKI_DUST || item == I_GAKI_FINGER)
	*temp += " gaki";
      if (item == I_DRAGON_CLAW || item == I_DRAGON_SCALES || item == I_DRAGON_BLOOD)
	*temp += " dragon";
      *temp += " and may be used in the creation of artifacts.";
    }
    //end BUSHIDO mod
    

    //BUSHIDO mods
    //if (item == I_KYOKUTSU_SHOGEI)  
    //  *temp += " This weapon gives a bonus of +1 to the unit's Stealth "
    //	"ability when carried.";
    //emd BUSHIDO mods
    if (item == I_BANNER_COMMAND)  
      *temp += " This weapon gives a bonus of +1 to the unit's Tactics "
	"ability when carried.";
    if (item == I_JEWEL_KAMI)  
      *temp += " This weapon gives a bonus of +3 to the unit's Observation "
	"ability when carried.";
    if (item == I_EYE_KAMI)  
      *temp += " This weapon gives a bonus of +2 to the unit's Observation "
	"ability when carried.";
    if (item == I_GEM_KAMI)  
      *temp += " This weapon gives a bonus of +1 to the unit's Observation "
	"ability when carried.";
    if (item == I_NINJA_GI_MASTERWORK)  
      *temp += " This armor gives a bonus of +1 to the unit's Stealth "
	"ability when worn.";
    if (item == I_NINJA_GI_MAGICAL)  
      *temp += " This armor gives a bonus of +2 to the unit's Stealth "
	"ability when worn.";
    if (item == I_KIMONO_GAKI)  
      *temp += " This armor gives a bonus of +3 to the unit's Stealth "
	"ability when worn.";
    if (item == I_PORTAL)
      *temp += " This is a magical item that is used to create a spacial link"
	" between two regions. Units may travel between the two regions"
	" provided each region has an active portal in place.";
    //emd BUSHIDO mods
	...
	//BUSHIDO mods
	if (pW->flags & WeaponType::W_BUSHIDO_NINJA)
	  *temp += " This item can only be used by a ninja.";
	if (pW->flags & WeaponType::W_BUSHIDO_SAMURAI)
	  *temp += " This item can only be used by a samurai.";
	if (pW->flags & WeaponType::W_BUSHIDO_MERCHANT)
	  *temp += " This item can only be used by a merchant.";
	if (pW->flags & WeaponType::W_BUSHIDO_SORCEROR)
	  *temp += " This item can only be used by a sorceror.";
	//END BUSHIDO mods
	...
	*temp += WeapType(pW->flags, pW->weapClass) + " weapon.";
	...
	//BUSHIDO assassinate mod
	if(pW->flags & WeaponType::W_USEINASSASSINATE) {
	  *temp += " This weapon may be used during assassination "
	    "attempts.";
	} else {
	  *temp += " This weapon cannot be used during assassination "
	    "attempts.";
	}
	//END BUSHIDO mods
	...
	//BUSHIDO mods
	if (pA->flags & ArmorType::A_BUSHIDO_NINJA)
	  *temp += " This item can only be used by a ninja.";
	if (pA->flags & ArmorType::A_BUSHIDO_SAMURAI)
	  *temp += " This item can only be used by a samurai.";
	if (pA->flags & ArmorType::A_BUSHIDO_MERCHANT)
	  *temp += " This item can only be used by a merchant.";
	if (pA->flags & ArmorType::A_BUSHIDO_SORCEROR)
	  *temp += " This item can only be used by a sorceror.";
	//END BUSHIDO mods
	...
	*temp += ".";
    ...
    //BUSHIDO MODS
    if (ItemDefs[item].flags & ItemType::NEEDBUILDING) {
      *temp += " This item must be produced in a ";
      *temp += ObjectDefs[ItemDefs[item].pBuilding].name;
      *temp += ".";
    }
