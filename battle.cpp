...
{
  //BUSHIDO mods
  //samurai cause fear mod
  if (a->unit->type == U_BUSHIDO_SAMURAI) {
      DoSpecialSamuraiAttack(round, a, attackers, def, behind, ass);
  }
  //end BUSHIDO mods
  ...
  if (!behind && (a->riding != -1)) {
	...
	num  = def->DoAnAttack(pMt->mountSpecial, realtimes,
			       ...
			       spd->damage[i].effect, 0, a->attackReadyness, ass);
	if(num != -1) {
	  if(tot == -1) tot = num;
	  else tot += num;
	}
  ...
  for (int i = 0; i < numAttacks; i++) {
    ...
    def->DoAnAttack(NULL, 1, attackType, a->askill, flags, attackClass,
		    NULL, mountBonus, a->attackReadyness, ass);
...
void Battle::GetSpoils(AList * losers, ItemList *spoils, int ass)
{
  ...
  forlist(losers) {
    ...
    forlist(&u->items) {
      Item * i = (Item *) elem;
      //BUSHIDO hack for monster parts
      if(IsMonster(i->type)) {
	//get monster parts!
	int chance = 2;
		 ...
		 ARegionList *pRegs )
{
  ...
  if (ass) {
    ...
    //BUSHIDO MOD TERRAIN ADVANTAGE - adds to TACT skill
    //if ((armies[0]->tac + armies[0]->gain_adv) > 
    //    (armies[1]->tac + armies[1]->gain_adv)) 
    if (armies[0]->tac > armies[1]->tac)
      FreeRound(armies[0],armies[1]);
    else if (armies[0]->tac < armies[1]->tac)
      //      if ((armies[1]->tac + armies[1]->gain_adv) > 
      //    (armies[0]->tac + armies[0]->gain_adv)) 
