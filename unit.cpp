...
Unit::Unit()
{
    ...
    //BUSHIDO mod
    //forced march check
    forcedMarchBonus = GetForcedMarchBonus();
    if (type == U_BUSHIDO_NINJA) {
      water = GetWaterWalkingBonus();
      wilderness = GetWildernessMovementBonus();
    } else {
      water = 0;
      wilderness = 0;
    }
		       ...
		       int autosee, int attitude)
{
    ...
    //BUSHIDO - check this here
    //if (obs == 2 && (type == U_MAGE || type == U_GUARDMAGE)) {
    if (obs == 2 && (type == U_MAGE || type == U_GUARDMAGE
		     || type == U_BUSHIDO_NINJA || type == U_BUSHIDO_SAMURAI
		     || type == U_BUSHIDO_SORCEROR || type == U_BUSHIDO_MERCHANT)) {
	temp += MageReport();
    }
...
int Unit::GetSkill(int sk)
{
    ...
    if (sk == S_GAMBLING) return GetAttribute("gambling");
    int retval = GetAvailSkill(sk);
    return retval;
}


int Unit::GetRealSkillDays(int sk)
{
  int retval = skills.GetDays(sk);
  return retval;
}
...
int Unit::CanStudy(int sk)
{
	...
	//	if ((sk == S_ILLUSION) && (type == U_APPRENTICE) && (Globals->APPRENTICE_MAGIC_LEVEL == Globals->ILLU_ONLY)) {
	//    SkillDepend *sd = new SkillDepend();
	//    sd[0].skill = "MANI";
	//    sd[0].level = 1;
	//    SkillType *pS = FindSkill("MANI");
	//    if (pS && (pS->flags & SkillType::DISABLED)) continue;
	//    if (!CheckDepend(curlev, sd[0])) return 0;
	//    else return 1;
	//} else {
	SkillType *pS = FindSkill(SkillDefs[sk].depends[c].skill);
	if (pS && (pS->flags & SkillType::DISABLED)) continue;
...
int Unit::Study(int sk, int days)
{
    ...
    //if((SkillDefs[sk].flags & SkillType::MAGIC) && type == U_APPRENTICE)
    //    max = 1;
    //}
    //end POON modification
...
int Unit::MaintCost()
{
    ...
    if (Globals->MULTIPLIER_USE != GameDefs::MULT_NONE) {
      if (type == U_BUSHIDO_SAMURAI
	  || type == U_BUSHIDO_MERCHANT
	  || type == U_BUSHIDO_NINJA
	  || type == U_BUSHIDO_SORCEROR) {
...
int Unit::MoveType()
{
    int weight = items.Weight();
    //BUSHIDO mods
    //    if (this->type == U_BUSHIDO_NINJA) {
    //  return GetNinjaMovementType(weight,TerrainDefs[object->region->type].similar_type);
    //} 
    //end BUSHIDO mods
...
int Unit::CalcMovePoints()
{
	...
	case M_NONE:
	    return 0;
	case M_WALK:
	    //BUSHIDO forced march
	    return Globals->FOOT_SPEED + forcedMarchBonus;
	    //END BUSHIDO mod
	case M_RIDE:
	    return Globals->HORSE_SPEED + forcedMarchBonus;
	case M_FLY:
...
int Unit::GetArmor(AString &itm, int ass)
{
    ...
    //BUSHIDO armour check for classes - test
    if ((pa->flags & ArmorType::A_BUSHIDO_NINJA) ||
	//(pa->flags & ArmorType::A_BUSHIDO_GEISHA) ||
	//(pa->flags & ArmorType::A_BUSHIDO_KUGE) ||
	(pa->flags & ArmorType::A_BUSHIDO_SAMURAI) ||
	(pa->flags & ArmorType::A_BUSHIDO_MERCHANT) ||
	//(pa->flags & ArmorType::A_BUSHIDO_ARTISAN) ||
	//(pa->flags & ArmorType::A_BUSHIDO_PRIEST) ||
	(pa->flags & ArmorType::A_BUSHIDO_SORCEROR)) {
	if (!(pa->flags & this->type))
	    return -1;
    }
    //end BUSHIDO check
...
int Unit::GetMount(AString &itm, int canFly, int canRide, int &bonus, int assassination)
{
    bonus = 0;

    //no mounts used in assassination
    if (assassination == 1)
	return -1;
		    ...
		    int assassination, int &ranged)
{
    ...
    // Found a weapon, check flags and skills - check this
    int baseSkillLevel = CanUseWeapon(pWep, riding, assassination);
    // returns -1 if weapon cannot be used, else the usable skill level
    if(baseSkillLevel == -1) return -1;

    //is it a ranged weapon?
    if (pWep->attackType == ATTACK_RANGED)
      ranged = 1;
    else
      ranged = 0;
...
int Unit::GetAttribute(char *attrib)
{
	...
	if (ap->mods[index].flags & AttribModItem::SKILL) {
	    ...
	    //BUSHIDO mods
	    if (temp == "searching") {
	      int skill = GetSkill(S_KEEN_EYES);
	      int flag = GetFlag(FLAG_SEARCHING);
	      if (skill > 0 && flag == FLAG_SEARCHING)
		val = (skill/2)+1;
	      else
		val = 0;
	    }
	    if (temp == "camouflage") {
	      int skill = GetSkill(S_CAMOUFLAGE);
	      int flag = GetFlag(FLAG_CAMOUFLAGE);
	      if (skill > 0 && flag == FLAG_CAMOUFLAGE)
		val = (skill/2)+1;
	      else
		val = 0;
	    }
...
int Unit::CanUseWeapon(WeaponType *pWep, int riding, int assassination)
{
    if (riding == -1) {
	if(pWep->flags & WeaponType::NOFOOT) return -1;
    }
    else if(pWep->flags & WeaponType::NOMOUNT) return -1;
    return CanUseWeapon(pWep,assassination);
}

int Unit::CanUseWeapon(WeaponType *pWep, int assassination)
{
    ...
    //assassination check
    if (assassination == 1 && !(pWep->flags & WeaponType::W_USEINASSASSINATE))
	return -1;

    //BUSHIDO class check - TEST
    //check whether weapons can be used by classes
    if ((pWep->flags & WeaponType::W_BUSHIDO_NINJA) ||
	//(pWep->flags & WeaponType::W_BUSHIDO_GEISHA) ||
	//(pWep->flags & WeaponType::W_BUSHIDO_KUGE) ||
	(pWep->flags & WeaponType::W_BUSHIDO_SAMURAI) ||
	(pWep->flags & WeaponType::W_BUSHIDO_MERCHANT) ||
	//(pWep->flags & WeaponType::W_BUSHIDO_ARTISAN) ||
	//(pWep->flags & WeaponType::W_BUSHIDO_PRIEST) ||
	(pWep->flags & WeaponType::W_BUSHIDO_SORCEROR)) {
      if (!(pWep->flags & this->type))
	return -1;
    }
    //end BUSHIDO class check
...
int Unit::GetHits() {
    int hits = 1; //default hits for a leader/non leader unit
    int level = GetSkill(S_ENDURANCE); //allows for multiple hits
...
int Unit::GetForcedMarchBonus() {

    int skill = GetSkill(S_FORCED_MARCH);
    if (skill == 0) {
	return 0;
    } else {
	//20% chance of bonus movement per level
	//0-49: 0
	//50-99: 1 MP bonus
	//100+: 2 MP bonus
	
	int chance = getrandom(100) + (skill * 20);
	if (chance > 99)
	    return 2;
	else
	    return 0;
...
int Unit::GetWaterWalkingBonus() {

    int skill = GetSkill(S_WATER_WALKING);
    int chance = 0;
    if (skill == 0) {
	return 0;
    } else {
      chance = getrandom(100) + (skill * 20);
    }
...
int Unit::GetWildernessMovementBonus() {

    int skill = GetSkill(S_WILDERNESS_MOVEMENT);
    int chance = 0;
    if (skill == 0) {
	return 0;
    } else {
...
int Unit::GetNinjaMovementType(int weight, int terrain) {

  //check the ninja movement types that are available
  int water = GetSkill(S_WATER_WALKING);
  int wilderness = GetSkill(S_WILDERNESS_MOVEMENT);
  int chance = 0;

  //water walking check
  if (water > 0 && terrain == R_OCEAN) {
    //check water walking chance
    chance = getrandom(100) + water*20;
    if (chance >= 100) {
      //check wilderness movement
      if (wilderness > 0) {
	chance = getrandom(100)+ wilderness*20;
	if (chance >= 100) {
	  if (CanWalk(weight))
	    return M_FLY;
	} else {
