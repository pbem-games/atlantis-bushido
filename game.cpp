...
void Game::MonsterCheck(ARegion *r, Unit *u)
{
  ...
  if (u->type != U_WMON) {
	...
	for (number=0; number<creatures;number++) {
	  if (getrandom(100) < (ItemDefs[i->type].esc_val+1))
	    decay++;
	}

	int newamount = i->num - decay;
	if (newamount < 0)
	  newamount = 0;

	u->items.SetNum(i->type,newamount);
	u->Event(ItemString(i->type, decay) +
	...
	// ESC_LEV_SQUARED or ESC_LEV_QUAD or ESC_LEV
	tmp = ItemDefs[i->type].esc_skill;
	skill = LookupSkill(&tmp);
	...
	if (!level) chance = 10000;
	  ...
	  else if (ItemDefs[i->type].escape & ItemType::ESC_LEV)
	    bottom = level;
	  else
	    bottom = 1;
	...
	if (ItemDefs[i->type].escape & ItemType::LOSE_LINKED) {
	  ...
	  //BUSHIDO mod
	  //calculate how many are lost - random, min 1.
	  int max = i->num;
	  int lost = getrandom(max);
	  int remaining = 0;
	  if (lost < 0)
	    lost = 1;
	  if (lost > max)
	  ...
	  u->Event(AString("Loses control of ") +
		   ItemString(i->type,lost) + ".");
	  u->items.SetNum(i->type,remaining); 
	  //u->Event(AString("Loses control of ") +
	  //     ItemString(i->type, i->num) + ".");
	  //u->items.SetNum(i->type, 0);
	  //end BUSHIDO mod
...
void Game::CreateCityMon(ARegion *pReg, int percent, int needmage)
{
  ...
  */
  ...
  if((Globals->LEADERS_EXIST) || (pReg->type == R_NEXUS)) {
    ...
    u->items.SetNum(I_NAGINATA,num);
    //u->items.SetNum(I_ASHIGURU_ARMOUR,num);
  ...
  u->items.SetNum(I_ASHIGURU_ARMOUR, num);
  u->SetSkill(S_TACTICS,4);
  u->SetSkill(S_OBSERVATION,4);

  //if (AC) {
  //	if(Globals->START_CITY_GUARDS_PLATE) {
  //	    if(Globals->LEADERS_EXIST) u->items.SetNum(I_ASHIGURU_ARMOUR, num);
  //	} else {
  //	    if(Globals->LEADERS_EXIST) u->items.SetNum(I_LEATHER_ARMOUR, num);
  //	}
