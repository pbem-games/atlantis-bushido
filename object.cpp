...
AString *ObjectDescription(int obj)
{
  ...
  *temp += AString(o->name) + ": ";
  ...
  //BUSHIDO fix for prerequisite buildings
  if (o->flags & ObjectType::NEEDBUILDING) {
    *temp += " This structure requires the following buildings to be"
      " present in the region before it can be built: ";
	  
    for (int i=0; i<4; i++) {
      int building = o->prerequisiteBuildings[i];
      if (building != -1) {
	if (i!=0)
	  *temp += ", ";
	      
	//print out building info
	if (building == O_RANCH_OR_FARM) {
	  *temp += "farm or ranch";
	} else {
	  *temp += ObjectDefs[building].name;
	}
      }	  
    }
    *temp += ".";
  ...
  //OBSE vs assassination
  int bonus = o->observationBonus;
  if (bonus > 0) {
    *temp += " This structure also imparts a bonus of +";
    *temp += bonus;
    *temp += " to the Observation skill of all units in the";
    *temp += " structure during assassination attempts.";

    if (obj == O_WATCHTOWER || obj == O_GEISHA_HOUSE) {
      *temp += " In addition, this structure gives an Observation";
      *temp += " bonus of +"; 
      *temp += bonus; 
      *temp += " to all units in the structure to";
      *temp += " determine which units are present in this region";
      *temp += " at the end of the month.";
    }
  }

  //building limits
  if (obj == O_GAMBLING_DEN) {
      *temp += " Only one gambling den can built in a region.";
  }
  ...
  if(o->maxMages && Globals->LIMITED_MAGES_PER_BUILDING) {
	...
	" samurai to study above level 2.";
    } else if (obj == O_NINJA_RYU) {
      *temp += AString(" This structure will allow up to ") + o->maxMages +
	" ninja to study above level 2.";
    } else if (obj == O_TOWER) {
      *temp += AString(" This structure will allow up to ") + o->maxMages +
	" sorcerors to study above level 2.";
    } else if (obj == O_YAKUZA_HOUSE) {
      *temp += AString(" This structure will allow up to ") + o->maxMages +
	" merchants to study above level 2.";
    }
  }

  if (obj == O_FORGE) {
    *temp += AString(" This structure allows the production of masterwork items and steel. To produce these items, the unit must be insdide the building.");
  } else if (obj == O_MAGICAL_FORGE) {
    *temp += AString(" This structure allows the production of magical items and artifacts. To produce these items, the unit must be insdide the building.");
  }

  if (o->regionalMax > 0) {
    *temp += AString(" A maximum of ") + o->regionalMax + " " + o->name + " can be built in a region.";
  }
