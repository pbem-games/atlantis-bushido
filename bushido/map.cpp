...
void ARegionList::CreateAbyssLevel(int level, char *name)
{
  ...
  o->name = new AString(AString(ObjectDefs[O_CITADEL].name)+" ["+o->num+"]");
  o->type = O_CITADEL;
  o->incomplete = 0;
  o->inner = -1;
  lair->objects.Add(o);
...
void ARegionList::CreateSpiritLevel(int level, int xSize, int ySize, char *name)
{
  MakeRegions(level, xSize, ySize);
  pRegionArrays[level]->SetName(name);
  pRegionArrays[level]->levelType = ARegionArray::LEVEL_NEXUS;
  ...
  AString nex_name = Globals->WORLD_NAME;
  nex_name += " Nexus";
  AString spirit_name = Globals->WORLD_NAME;
  spirit_name += " Spirit World";
  ...
  for(y = 0; y < ySize; y++) {
    for(int x = 0; x < xSize; x++) {
      ARegion *reg = pRegionArrays[level]->GetRegion(x, y);
      if(reg && Globals->NEXUS_IS_CITY && Globals->TOWNS_EXIST && reg->type == R_NEXUS) {
	reg->MakeStartingCity();
	if(Globals->GATES_EXIST) {
	  numberofgates++;
	}
