...
void Battle::DoSpecialSamuraiAttack(int round, Soldier *a, Army *attackers,
			     Army *def, int behind, int assassination)
{
  SpecialType *spd;
  int i, num, tot = -1;
  AString results[4];
  int dam = 0;
  int level = a->unit->GetSkill(S_CREATE_FEAR);
  if (level == 0) return;

  if (assassination == 1) return;

  SkillType *pST = &SkillDefs[S_CREATE_FEAR];
  char* special = pST->special;
  
  if(special == NULL) return;
  spd = FindSpecial(special);
  ...
  for(i = 0; i < 4; i++) {
    ...
    int realtimes = spd->damage[i].minnum + getrandom(times) +
      getrandom(times);
    num = def->DoAnAttack(a->special, realtimes,
			  spd->damage[i].type, level,
			  spd->damage[i].flags, spd->damage[i].dclass,
			  spd->damage[i].effect, 0, 0, assassination);
    if(spd->effectflags & SpecialType::FX_DONT_COMBINE && num != -1) {
      if(spd->damage[i].effect == NULL) {
	results[dam] = AString("killing ") + num;
	dam++;
