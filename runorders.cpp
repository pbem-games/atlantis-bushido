...
void Game::ClearCastEffects()
{
	...
	forlist(&r->objects) {
		...
		u->SetFlag(FLAG_SEARCHING, 0);
		u->SetFlag(FLAG_CAMOUFLAGE, 0);
	    }
	}
    }
}
...
void Game::RunForgetOrders()
{
	...
	forlist(&r->objects) {
		...
		forlist(&u->forgetorders) {
		    ...
		    //partial forget modification
		    if (fo->days == 450) {
		      u->ForgetSkill(fo->skill);
		      u->Event(AString("Forgets ") + SkillStrs(fo->skill) + ".");
		    } else {
		      int current = u->GetRealSkillDays(fo->skill);
		      if (current - fo->days > 1) {
			u->SetSkillDays(fo->skill,(current-(fo->days)));
			u->Event(AString("Forgets ") + fo->days + AString(" days of ") + SkillStrs(fo->skill) + ".");
		      } else {
			u->ForgetSkill(fo->skill);
			u->Event(AString("Forgets ") + SkillStrs(fo->skill) + ".");
		      }
...
void Game::RunPillageRegion(ARegion *reg)
{
    ...
    if (pillagers * Globals->MIN_MEN_PILLAGE_MULT * Globals->TAX_BASE_INCOME < reg->wealth) {
	ClearPillagers(reg);
	return;
    }

    AList *facs = reg->PresentFactions();
    int amt = reg->wealth * Globals->TAX_PILLAGE_MULT;
    forlist(&reg->objects) {
	Object *o = (Object *) elem;
	forlist(&o->units) {
	    Unit *u = (Unit *) elem;
...
void Game::RunRazeOrders()
{
    forlist (&regions) {
	RunRazeRegion((ARegion *) elem);
    }
}

int Game::CountRazers(ARegion *reg)
{
    int p = 0;
    forlist(&reg->objects) {
	Object *o = (Object *) elem;
	forlist(&o->units) {
		...
		if (!reg->CanPillage(u)) {
		    u->Error("RAZE: A unit is on guard.");
		    u->taxing = TAX_NONE;
		} else {
		    int men = u->Taxers(1);
		    if (men) {
			if(!TaxCheck(reg, u->faction)) {
			    u->Error("RAZE: Faction can't tax that many "
				     "regions.");
			    u->taxing = TAX_NONE;
			} else {
			    p += men;
			...
			u->Error("RAZE: Not a combat ready unit.");
			u->taxing = TAX_NONE;
		    }
		}
	    }
...
void Game::ClearRazers(ARegion *reg)
{
    forlist(&reg->objects) {
	Object *o = (Object *) elem;
	forlist(&o->units) {
	    ...
	    if (u->taxing == TAX_RAZE) {
		if (&reg->town) {
		    u->Error("RAZE: Not enough men to raze.");
		    u->taxing = TAX_NONE;
		} else {
		    u->Error("RAZE: No settlement to raze.");
		    u->taxing = TAX_NONE;
		}
	    }
	}
...
void Game::ClearRazersNoSettlement(ARegion *reg)
{
    forlist(&reg->objects) {
	Object *o = (Object *) elem;
	forlist(&o->units) {
...
void Game::RunRazeRegion(ARegion *reg)
{
    if (!reg->town) {
	ClearRazersNoSettlement(reg);
	return;
    }
    ...
    /* First, count up razers */
    int razers = CountRazers(reg);

    if ((razers * Globals->MIN_MEN_RAZE_MULT * Globals->TAX_BASE_INCOME < reg->wealth) || !reg->town) {
	ClearRazers(reg);
	return;
    }

    AList *facs = reg->PresentFactions();
    int amt = reg->wealth * Globals->TAX_RAZE_MULT;
    forlist(&reg->objects) {
	Object *o = (Object *) elem;
	forlist(&o->units) {
	    Unit *u = (Unit *) elem;
		...
		u->taxing = TAX_NONE;
		int num = u->Taxers(1);
		int temp = (amt * num)/razers;
		amt -= temp;
		razers -= num;
		u->SetMoney(u->GetMoney() + temp);
		u->Event(AString("Razes $") + temp + " from " +
			 reg->ShortPrint(&regions) + ".");
		forlist(facs) {
		    Faction *fp = ((FactionPtr *) elem)->ptr;
		    if (fp != u->faction) {
			fp->Event(*(u->name) + " razes " +
				  *(reg->name) + ".");
		    }
		}
	    }
...
void Game::DoSell(ARegion *r, Market *m)
{
	...
	forlist((&obj->units)) {
		...
		if (o->item == m->item) {
		    ...
		    //BUSHIDO mod
		    int commerce = u->GetSkill(S_COMMERCE);
		    if (commerce > 0) {
		      int bonus = 5 * commerce;
		      u->SetMoney(u->GetMoney() + (int)(temp * m->price * bonus / 100));
		    } else {
...
int Game::GetBuyAmount(ARegion *r, Market *m)
{
	...
	forlist((&obj->units)) {
		...
		if (o->item == m->item) {
			...
			//BUSHIDO mods - FIX HERE
			if (u->type == U_BUSHIDO_SAMURAI ||
			    u->type == U_BUSHIDO_KUGE ||
			    u->type == U_BUSHIDO_MERCHANT ||
			    u->type == U_BUSHIDO_ARTISAN ||
			    u->type == U_BUSHIDO_PRIEST ||
			    u->type == U_BUSHIDO_SORCEROR ||
			    u->type == U_BUSHIDO_NINJA ||
			    u->type == U_BUSHIDO_GEISHA) {
			    u->Error("BUY: Specialsts can't recruit more men.");
			    o->num=0;
			}
			//BUSHIDO MODS end
			//if (u->type == U_MAGE) {
...
void Game::DoBuy(ARegion *r, Market *m)
{
	...
	forlist((&obj->units)) {
		...
		if (o->item == m->item) {
		    ...
		    //BUSHIDO mod
		    int commerce = u->GetSkill(S_COMMERCE);
		    int cost = temp * m->price;
		    int saving = 0;
		    if (commerce > 0) {
		      int bonus = 5 * commerce;
		      saving = (int)(m->price * temp * bonus /100);
		    } 
		    u->ConsumeSharedMoney(cost-saving);
		    //end BUSHIDO mod
		    u->buyorders.Remove(o);
...
int Game::DoGiveOrder(ARegion *r, Unit *u, GiveOrder *o)
{
	...
	//BUSHIDO WAR
      if (u->type == U_BUSHIDO_SAMURAI /*|| u->type == U_BUSHIDO_KUGE*/) {
	    if (CountWar(t->faction) >= AllowedWar(t->faction)) {
		u->Error("GIVE: Faction has too many war specialists.");
		return 0;
	    }
	}

	//BUSHIDO TRADE
	if (/*u->type == U_BUSHIDO_ARTISAN ||*/ u->type == U_BUSHIDO_MERCHANT) {
	    if (CountTrade(t->faction) >= AllowedTrade(t->faction)) {
		u->Error("GIVE: Faction has too many trade specialists.");
		return 0;
	    }
	}

	//BUSHIDO MAGIC
	if (/*u->type == U_BUSHIDO_PRIEST ||*/ u->type == U_BUSHIDO_SORCEROR) {
	    if (CountMagic(t->faction) >= AllowedMagic(t->faction)) {
		u->Error("GIVE: Faction has too many magic specialists.");
		return 0;
	    }
	}
	...
	if (u->type == U_BUSHIDO_NINJA /*|| u->type == U_BUSHIDO_GEISHA*/) {
	    if (CountStealth(t->faction) >= AllowedStealth(t->faction)) {
		u->Error("GIVE: Faction has too many stealth specialists.");
		return 0;
	    }
	}
	//END BUSHIDO specialist check
	...
	/*if (u->type == U_MAGE) {
	  ...
	  if (u->GetSkill(S_STEALTH) == 5) {
	  if (Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_FACTION_TYPES) {
	  if (Globals->ASSASSINS_NEED_STEALTH) {
	  
	  if (CountAssassins(t->faction) >=
	  AllowedAssassins(t->faction)) {
	  u->Error("GIVE: Faction has too many assassins.");
	  return 0;
	  }
	  }
	  }
	...
	*/
	...
	//BUSHIDO unit type check
	if (u->type == U_BUSHIDO_SAMURAI || t->type == U_BUSHIDO_SAMURAI
	    //|| u->type == U_BUSHIDO_KUGE || t->type == U_BUSHIDO_KUGE
	    || u->type == U_BUSHIDO_NINJA || t->type == U_BUSHIDO_NINJA
	    //|| u->type == U_BUSHIDO_GEISHA || t->type == U_BUSHIDO_GEISHA
	    //|| u->type == U_BUSHIDO_ARTISAN || t->type == U_BUSHIDO_ARTISAN
	    || u->type == U_BUSHIDO_MERCHANT || t->type == U_BUSHIDO_MERCHANT
	    //|| u->type == U_BUSHIDO_PRIEST || t->type == U_BUSHIDO_PRIEST
	    || u->type == U_BUSHIDO_SORCEROR || t->type == U_BUSHIDO_SORCEROR) {

	    u->Error("GIVE: Specialists can't transfer men.");
	    return 0;
	}
...
void Game::CheckTransportOrders()
{
	...
	forlist((&r->objects)) {
		...
		forlist ((&u->transportorders)) {
			...
			if (tar->unit->GetSkill(S_CARAVANMASTER == 0)) {	    
			    u->Error(ordertype + 
				     ": Target is not a Merchant specialist");
			    o->type = NORDERS;
			    continue;
			}
			...
			if (maxdist >= 0 &&
				...
				level = u->GetSkill(S_CARAVANMASTER);
			    //else if (u->type == U_BUSHIDO_KUGE)
			    //level = u->GetSkill(S_QUARTERMASTER);
			    maxdist += ((level + 1)/3);
			} else if (maxdist == 0)
			    maxdist = 10000000;
		    } else
			...
			((dist > Globals->LOCAL_TRANSPORT) &&
			 (o->type == O_TRANSPORT))) {
			if (u->GetSkill(S_CARAVANMASTER) == 0) {
			    u->Error(ordertype +
				     ": Unit is not Merchant specialist.");
			    o->type = NORDERS;
			    continue;
			}
			if ((u != obj->GetOwner()) ||
...
void Game::CheckDistributeOrders()
{
    if (!(Globals->TRANSPORT & GameDefs::ALLOW_TRANSPORT))
	return;
	...
	forlist((&r->objects)) {
		...
		forlist ((&u->transportorders)) {
		    // make sure target exists
		    DistributeOrder *o = (DistributeOrder *)elem;
		    AString ordertype = O_DISTRIBUTE;
		    if (!o->target || o->target->unitnum == -1) {
			u->Error(ordertype + ": Target does not exist.");
			o->type = NORDERS;
			continue;
		    ...
		    // On distribute, make sure the
		    // issuer is a quartermaster and is owner of a structure
		    if (o->type == O_DISTRIBUTE) {
			if (u->GetSkill(S_CARAVANMASTER == 0)) {
			    u->Error(ordertype +
				     ": Unit is not a Merchant specialist.");
			    o->type = NORDERS;
			    continue;
			}
		    ...
		    // Check if we have a trade hex
		    if (!TradeCheck(r, u->faction)) {
			u->Error(ordertype + ": Faction cannot distribute in that many hexes.");
			o->type = NORDERS;
			continue;
		    }
		}
...
void Game::RunTransportOrders()
{
	...
	forlist((&r->objects)) {
		...
		forlist ((&u->transportorders)) {
		    ...
		    if (t->type != O_TRANSPORT)
			continue;
		    Location *tar = regions.GetUnitId(t->target,
						      u->faction->num, r);
		    if (!tar) continue;
		    AString ordertype = "TRANSPORT";
		    int amt = t->amount;
		    if (amt < 0) {
			...
			if (Globals->TRANSPORT & GameDefs::QM_AFFECT_COST) {
			  ...
			  /*else*/ if (u->GetSkill(S_CARAVANMASTER) > 0)
			    cost *= (4 - ((u->GetSkill(S_CARAVANMASTER)+1)/2));
			}
		    }
...
void Game::RunDistributeOrders()
{
	...
	forlist((&r->objects)) {
	    ...
	    forlist((&obj->units)) {
		Unit *u = (Unit *)elem;
		forlist ((&u->distributeorders)) {
		    DistributeOrder *t = (DistributeOrder *)elem;
		    if (t->type != O_DISTRIBUTE)
			continue;
		    Location *tar = regions.GetUnitId(t->target,
						      u->faction->num, r);
		    if (!tar) continue;
		    AString ordertype = "DISTRIBUTE";
