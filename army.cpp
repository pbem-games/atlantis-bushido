...
Soldier::Soldier(Unit * u,Object * o,int regtype,int r,int ass)
{
  ...
  //BUSHIDO endurance modification
  hits = u->GetHits();
  maxhits = u->GetHits();
  //end BUSHIDO endurance modification
  ...
  if(canFly || canRide) {
    ...
    for(mountType = 1; mountType < NUMMOUNTS; mountType++) {
      ...
      item = unit->GetMount(abbr, canFly, canRide, ridingBonus, ass);
      if(item == -1) continue;
  ...
  for(i = 0; i < MAX_READY; i++) {
    ...
    item = unit->GetWeapon(abbr, riding, ridingBonus, attackBonus,
			   defenseBonus, numAttacks, ass, ranged);
    if(item != -1) {
      weapon = item;
      break;
    }
  ...
  //BUSHIDO MODS
  //BUSHIDO Samuarai
  //update number of attacks + deflect arrows + resist fear
  if (unit->type == U_BUSHIDO_SAMURAI) {
    resistFear = unit->GetSkill(S_RESIST_FEAR);
    attackReadyness = 5 * unit->GetSkill(S_WAY_SAMURAI);
    if (attackReadyness > 25)
      attackReadyness = 25;
    if (attackReadyness < 0)
      attackReadyness = 0;


    //skills only good with a melee weapon
    //only katanas, wakazashis and daisho
    if (!hands && !ranged) {

      //do check here for weapons
      deflectArrows = unit->GetSkill(S_DEFLECT_ARROWS);
      bonusAttacks = unit->GetSkill(S_MULTIPLE_ATTACKS);
    }
  }

  //ninja skills
  if (unit->type == U_BUSHIDO_NINJA) {
    resistEnergy = unit->GetSkill(S_RESIST_ELEMENTS);
    resistWeather = unit->GetSkill(S_RESIST_ELEMENTS);
    attackReadyness = 5 * unit->GetSkill(S_WAY_NINJA);
  ...
  //BUSHIDO Sorceror
  if (unit->type == U_BUSHIDO_SORCEROR) {
    resistEnergy = unit->GetSkill(S_SHIELD_LORE);
    resistFear = unit->GetSkill(S_SHIELD_LORE);
    deflectArrows = unit->GetSkill(S_SHIELD_LORE);
  }
  ...
  dskill[ATTACK_RANGED] += deflectArrows;
  dskill[ATTACK_WEATHER] += resistWeather;
  dskill[ATTACK_ENERGY] += resistEnergy;
  dskill[ATTACK_SPIRIT] += resistFear;
  attacks = numAttacks + bonusAttacks;
}
...
void Soldier::SetupSpell()
{
  ...
  if (!(unit->type == U_BUSHIDO_SORCEROR
      || unit->type == U_BUSHIDO_NINJA
      || unit->type == U_BUSHIDO_SAMURAI
	|| unit->type == U_BUSHIDO_MERCHANT))
    return;
  //end BUSHIUDO MOD
...
void Soldier::SetupCombatItems(int assassination)
{
  int battleType;
  int exclusive = 0;
  ...
  for(battleType = 1; battleType < NUMBATTLEITEMS; battleType++) {
    ...
    if (!(pBat->flags & BattleItemType::USEINASSASSINATE)
	&& assassination == 1)
      continue;
...
Army::Army(Unit * ldr,AList * locs,int regtype,int ass)
{
  ...
  //gain_adv = ldr->GetSkill(S_GAIN_ADVANTAGE);
  count = 0;
  hitstotal = 0;
  ...
  if (ass) {
	...
	tactician = u;
      ...
      //	  if (regtype == R_MOUNTAIN)
      //	      temp_gain = u->GetSkill(S_MOUNTAIN_ADVANTAGE);
      //	  else if (regtype == R_PLAIN)
      //	      temp_gain = u->GetSkill(S_PLAINS_ADVANTAGE);
      //	  else if (regtype == R_FOREST)
      //	      temp_gain = u->GetSkill(S_FOREST_ADVANTAGE);
      //	  else if (regtype == R_DESERT)
      //	      temp_gain = u->GetSkill(S_DESERT_ADVANTAGE);
      //	  
      //	  if (temp_gain > gain_adv)
      //	      gain_adv = temp_gain;
      //}
      //end BUSHIDO MOD
...
void Army::GetMonSpoils(ItemList *spoils,int monitem, int free)
{
  ...
  //BUSHIDO MOD
  int chance = 3;
  if (monitem == I_DAI_ONI) {
      chance = getrandom(chance);
      if (chance == 0) {
	  spoils->SetNum(I_ONI_BLOOD, spoils->GetNum(I_ONI_BLOOD) + 1);	      
      } else if (chance == 1) {
	  spoils->SetNum(I_ONI_HIDE, spoils->GetNum(I_ONI_HIDE) + 1);	      
      } else if (chance == 2) {
	  spoils->SetNum(I_ONI_HORN, spoils->GetNum(I_ONI_HORN) + 1);	      
      }
  } else if (monitem == I_MAJOR_KAMI) {
      chance = getrandom(chance);
      if (chance == 0) {
	  spoils->SetNum(I_KAMI_HAIR, spoils->GetNum(I_KAMI_HAIR) + 1);	      
      } else if (chance == 1) {
	  spoils->SetNum(I_KAMI_HEART, spoils->GetNum(I_KAMI_HEART) + 1);	      
      } else if (chance == 2) {
	  spoils->SetNum(I_KAMI_RIB, spoils->GetNum(I_KAMI_RIB) + 1);	      
      }
  } else if (monitem == I_GAKI) {
      chance = getrandom(chance);
      if (chance == 0) {
	  spoils->SetNum(I_GAKI_DUST, spoils->GetNum(I_GAKI_DUST) + 1);	      
      } else if (chance == 1) {
	  spoils->SetNum(I_GAKI_FINGER, spoils->GetNum(I_GAKI_FINGER) + 1);	      
      } else if (chance == 2) {
	  spoils->SetNum(I_GAKI_ICHOR, spoils->GetNum(I_GAKI_ICHOR) + 1);	      
      }
  } else if (monitem == I_DRAGON) {
      chance = getrandom(chance);
      if (chance == 0) {
	  spoils->SetNum(I_DRAGON_BLOOD, spoils->GetNum(I_DRAGON_BLOOD) + 1);	      
      } else if (chance == 1) {
	  spoils->SetNum(I_DRAGON_CLAW, spoils->GetNum(I_DRAGON_CLAW) + 1);	      
      } else if (chance == 2) {
	  spoils->SetNum(I_DRAGON_SCALES, spoils->GetNum(I_DRAGON_SCALES) + 1);	      
      }
  }
  //end BUSHIDO MOD
  ...
  //XXX: fix this up to give more random spoils
  for (i=0; i<NITEMS; i++) {
    if ((ItemDefs[i].type & thespoil) &&
	!(ItemDefs[i].type & IT_SPECIAL) &&
	!(ItemDefs[i].type & IT_ARTIFACT) &&
...
int Army::DoAnAttack(char *special, int numAttacks, int attackType,
		     ...
		     int mountBonus, int attackReadyness, int assassination)
{
  ...
  for(int i = 0; i < numAttacks; i++) {
    ...
    if (attackType != NUM_ATTACK_TYPES) {
      ...
      //BUSHIDO fix here...!
      //weapon is not always ready, so check....
      if(!(flags & WeaponType::ALWAYSREADY)) {
	int failchance = 2;
	int chance = getrandom(100);

	if(Globals->ADVANCED_FORTS && assassination == 0) {
	  failchance += (tar->protection[attackType]+1)/2;
	}

	//convert fail chance to %
	failchance = 100/failchance;

	//add attack readyness
	failchance -= attackReadyness;

	//check here - if less than, then do no attack
