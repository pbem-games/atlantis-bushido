...
void Faction::WriteReport(Areport *f, Game *pGame)
{
  ...
  //BUSHIDO unit types
  f->PutStr("");
  //	f->PutStr(AString("War specialists: ") + numKuge + AString(" Kuge, ") + numSamuarai + AString(" Samurai (") + pGame->AllowedWar(this) + AString(" maximum total)"));
  //f->PutStr(AString("Trade specialists: ") + numArtisan + AString(" Artisan, ") + numMerchant + AString(" Merchant (") + pGame->AllowedTrade(this) + AString(" maximum total)"));
  //f->PutStr(AString("Magic specialists: ") + numPriest + AString(" Priest, ") + numSorceror + AString(" Sorceror (") + pGame->AllowedMagic(this) + AString(" maximum total)"));
  //	f->PutStr(AString("Stealth specialists: ") + numGeisha + AString(" Geisha, ") + numNinja + AString(" Ninja (") + pGame->AllowedStealth(this) + AString(" maximum total)"));



  f->PutStr(AString("War specialists: ") + numSamuarai + AString(" Samurai (") + pGame->AllowedWar(this) + AString(" maximum total)"));
  f->PutStr(AString("Trade specialists: ") + numMerchant + AString(" Merchant (") + pGame->AllowedTrade(this) + AString(" maximum total)"));
  f->PutStr(AString("Magic specialists: ") + numSorceror + AString(" Sorceror (") + pGame->AllowedMagic(this) + AString(" maximum total)"));
  f->PutStr(AString("Stealth specialists: ") + numNinja + AString(" Ninja (") + pGame->AllowedStealth(this) + AString(" maximum total)"));
  f->PutStr("");
...
int Faction::CanSee(ARegion* r, Unit* u, int practice)
{
  ...
  forlist((&r->objects)) {
    ...
    forlist((&obj->units)) {
      ...
      if (Globals->ALLOW_BUILDING_OBSE == 1 && u->stealorders) {
	//get the object type and observation bonus
	if (u->stealorders->type == O_ASSASSINATE) {
	  ...
	  if (temp->num == tar->num) {
	    ...
	    stealpenalty = ObjectDefs[objType].observationBonus;
	  }
	}
      }
	...
	obs = temp->GetAttribute("observation");
	stea = u->GetAttribute("stealth") - stealpenalty;
	if (obs > stea) {
