...
void Game::MakeLMon(Object *pObj)
{
  ...
  case I_DAI_ONI:
    ...
    mp = FindMonster(ItemDefs[I_ONI].abr,
		     (ItemDefs[I_ONI].type & IT_ILLUSION));
    u->items.SetNum(I_ONI, getrandom(mp->number + 1));

    mp = FindMonster(ItemDefs[I_DAI_ONI].abr,
		     (ItemDefs[I_DAI_ONI].type & IT_ILLUSION));
    u->items.SetNum(I_DAI_ONI, getrandom(mp->number + 1));
  ...
  case I_KAMI:
  case I_MAJOR_KAMI:
    u->MakeWMon("Spirits", I_MINOR_KAMI, getrandom(mp->number + 1));

    mp = FindMonster(ItemDefs[I_KAMI].abr,
		     (ItemDefs[I_KAMI].type & IT_ILLUSION));
    u->items.SetNum(I_KAMI, getrandom(mp->number + 1));

    mp = FindMonster(ItemDefs[I_MAJOR_KAMI].abr,
		     (ItemDefs[I_MAJOR_KAMI].type & IT_ILLUSION));
    u->items.SetNum(I_MAJOR_KAMI, getrandom(mp->number + 1));
    break;
  case I_SKELETON:
  case I_UNDEAD:
  case I_GAKI:
    ...
    mp = FindMonster(ItemDefs[I_GAKI].abr,
		     (ItemDefs[I_GAKI].type & IT_ILLUSION));
    u->items.SetNum(I_GAKI, getrandom(mp->number + 1));
...
Unit *Game::MakeManUnit(Faction *fac, int mantype, int num, int level, int weaponlevel, int armor, int behind)
{
  ...
  */
  ...
  if(armor) {
    int ar = I_ASHIGURU_ARMOUR;
    if(!men->CanUse(ar)) ar = I_LEATHER_ARMOUR;
    //if(!men->CanUse(ar)) ar = I_LEATHER_ARMOUR;
