...
extern int *allowedAssassins;
extern int allowedAssassinsSize;
...
public:
	...
	// STEA 5 is limited by STEALTH point allocation
	int ASSASSINS_NEED_STEALTH;

	// Allow artifacts in the game
	int ENABLE_ARTIFACTS;

	// Allow apprentices to study ILLU spells, 1st level spells only etc...
	enum {
	  //apprentices can study MANI, plus all ILLU spells except INVI, TRUE.
	  ILLU_ONLY = 0x01,

	  //aprentices can study ALL magic up to level one only
	  ALL_MAGIC_LEVEL_ONE = 0x02,
	};
	int APPRENTICE_MAGIC_LEVEL;

	//Razing information
	//minimum multiplier for razing for no of men
	int MIN_MEN_RAZE_MULT;

	//tax multipler for razing
	int TAX_RAZE_MULT;

	//incremental raze
	//0 = off (all settlements -> ruin)
	//1 = on (city->town, town->village, village->ruin)
	int INCREMENTAL_RAZE;

	//Pillaging information
	//minimum multiplier for pillaging for no of men
	int MIN_MEN_PILLAGE_MULT;

	//tax multiplier for pillaging
	int TAX_PILLAGE_MULT;

	//min distance check for settlement creation
	int SETTLE_MIN_DISTANCE;

	//allow buildings to have an OBSE bonus/
	//STEA penalty versus stealing and assassination
	int ALLOW_BUILDING_OBSE;
