...
private:
	...
	void EditGameUnitName(Unit *pUnit);
	...
	void ProcessGambleOrder(Unit *, OrdersCheck *pCheck);
	...
	void ProcessRazeOrder(Unit *, OrdersCheck *pCheck);
	...
	int CountAssassins(Faction *);

	//Bushido mods
	int CountTrade(Faction *);
	int CountMagic(Faction *);
	int CountStealth(Faction *);
	int CountWar(Faction *);
	int AllowedTrade(Faction *);
	int AllowedMagic(Faction *);
	int AllowedStealth(Faction *);
	int AllowedWar(Faction *);
	//END Bushido mods
	...
	int GetSettlementStructures(ARegion *);
	int GetNearestSettlementDistance(ARegion *);
	void Do1SettleOrder(Unit *, ARegion *, AString *);
	void Do1GambleOrder(Unit *, Object *, int);

	//
	// Faction limit functions.
	//
	// The first 5 are game specific and can be found in extra.cpp. They
	...
	int AllowedAssassins(Faction *pFact);
	...
	void RunRazeOrders();
	int CountRazers(ARegion *);
	void ClearRazers(ARegion *);
	void ClearRazersNoSettlement(ARegion *);
	void RunRazeRegion(ARegion *);
	void RunTaxOrders();
	...
	void RunDistributeOrders();
	void CheckDistributeOrders();
