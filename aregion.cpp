...
void ARegion::LairCheck(int chance)
{
    // No lair if town in region
    if (town) return;
...
int ARegion::RazeCheck()
{
    int razeAdd = maxwages - wages;
    if (razeAdd > 0) return razeAdd;
    return 0;
}
...
int ARegion::GetObservation(Faction *f, int usepassers)
{
	...
	//BUSHIDO GEISHA HOUSE FIX - regional only
	int obseBonus = 0;
	int objType = obj->type;
	if ((objType == O_GEISHA_HOUSE) || (objType == O_WATCHTOWER))
	  obseBonus = ObjectDefs[objType].observationBonus;
	//END BUSHIDO FIX

	forlist ((&obj->units)) {
	    Unit *u = (Unit *) elem;
	    if (u->faction == f) {
		int temp = u->GetAttribute("observation") + obseBonus;
...
//BUSHIDO mod
int ARegion::IsObjectPresent(int type) {

    int needed[4] = {ObjectDefs[type].prerequisiteBuildings[0],
		     ObjectDefs[type].prerequisiteBuildings[1],
		     ObjectDefs[type].prerequisiteBuildings[2],
		     ObjectDefs[type].prerequisiteBuildings[3]};

    int found[4] = {1,1,1,1};

    //check for mutiple buildings
    for (int i=0; i<4; i++) {
	...
	forlist((&objects)) {
	    Object *o = (Object *) elem;
	    if (needed[i] == O_RANCH_OR_FARM) {
	      if (o->type == O_FARM) 
		found[i] = 1;
	      else if (o->type == O_RANCH)
		found[i] = 1;
	    } else {
	      if (o->type == needed[i]) {
		found[i] = 1;
...
int ARegion::RegionLimitReached(int type) {
  
  int max = ObjectDefs[type].regionalMax;
  int found = 0;
