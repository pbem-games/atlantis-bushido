...
/* Damage region because of razing */
void ARegion::Raze()
{
    wealth = 0;
    int damage = development / 4;
    development -= damage;
    int popdensity = Globals->CITY_POP / 2000;
    AdjustPop(- damage * getrandom(popdensity) - getrandom(5 * popdensity));
    /* Stabilise at minimal development levels */
    ...
    //check if city, town or village
    if (town) {
	if (Globals->INCREMENTAL_RAZE) {
	    if (town->TownType() == TOWN_CITY)
		SetTownType(TOWN_TOWN);
	    else if (town->TownType() == TOWN_TOWN)
		SetTownType(TOWN_VILLAGE);
	    else if (town->TownType() == TOWN_VILLAGE)
		town = NULL;
	} else {
	    town = NULL;
	}
    ...
    //destroy objects if they are here and are not lairs
    Object *dest = GetDummy();

    forlist (&objects) { 
	Object * obj = (Object *)elem;

	if (obj->type != O_DUMMY && obj->type != O_FLEET) {
	    //check here for other object types
	    forlist(&obj->units) {
		Unit *u = (Unit *) elem;
		u->MoveUnit(dest);  
	    }
    ...
    //reset building sequence to only have the DUMMY object
    buildingseq = 1;

    //if no town left, add monster lair
    if (!town) {
	LairCheck(1);
    }
}
