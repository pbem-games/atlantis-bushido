		...
		const AString &intro)
{
	...
	if (qm_exist) {
		/* Make sure the S_CARAVANMASTER skill is enabled */
		if (SkillDefs[S_CARAVANMASTER].flags & SkillType::DISABLED)
			qm_exist = 0;
	}
	int found = 0;
	if (qm_exist) {
	...
	temp += " at $";
	float ratio = ItemDefs[(Globals->RACES_EXIST?I_MAN:I_MAN)].baseprice/
	...
	temp = "Example: One man with a horse, yari, and leather armour wants to "
	...
	int weight = ItemDefs[I_MAN].weight + ItemDefs[I_YARI].weight +
			ItemDefs[I_LEATHER_ARMOUR].weight;
	temp += weight;
	if(cap > weight)
		temp += ", so he can ride";
	else
	...
	temp += SkillDefs[S_WAY_SORCEROR].cost;
	temp += ")";
	...
	temp = "A character enters the world of magic in Atlantis by beginning "
		"study of the War of the Sorceror skill.  Only one man units";
	//end BUSHIDO mods
